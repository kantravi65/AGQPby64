...
data class CandidateSession(
    ...
    var forceSubmitRequested: Boolean = false
)
...
object LiveTestState {
    ...
    fun setWarning(rollNumber: String, message: String) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                val nextWarn = it.warningCount + 1
                val newStatus = if (nextWarn >= 3) "Disqualified" else it.status
                it.copy(activeWarningMessage = message, warningCount = nextWarn, status = newStatus)
            } else {
                it
            }
        }
    ...
    var selectedExpertPaperId: String? = null
    ...
    fun updateWarnings(rollNumber: String, warnings: Int) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                val newStatus = if (warnings >= 3) "Disqualified" else it.status
                it.copy(warningCount = warnings, status = newStatus)
            } else {
                it
            }
        }
    ...
    fun updateCandidateAnswers(rollNumber: String, answersJson: String) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                it.copy(answersJson = answersJson)
            } else {
                it
            }
        }
    ...
    fun updateProgress(rollNumber: String, questionId: String, answersJson: String) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                it.copy(currentQuestionId = questionId, answersJson = answersJson)
            } else {
                it
            }
        }
    ...
    fun requestForceSubmit(rollNumber: String) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                it.copy(forceSubmitRequested = true)
            } else {
                it
            }
        }
    ...
    fun clearForceSubmit(rollNumber: String) {
        val list = _candidates.value.map {
            if (it.rollNumber == rollNumber) {
                it.copy(forceSubmitRequested = false)
            } else {
                it
            }
        }
